...
public class IsBalancedTree {
    ...
    public static int isBalanced(Node node) {
        ...
        int left_node = isBalanced(node.left);
        int right_node = isBalanced(node.right);

        int local_gap = Math.abs(left_node - right_node);

        if (local_gap > 1) {
            balanced = false;
        }

        int total = Math.max(left_node, right_node) + 1;
        return total;
